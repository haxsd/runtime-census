...
| 你要做什么 | 顺序 |
|---|---|
| **只是调用工具** | `find <工具>` 拿绝对路径 → 执行它。地图给的那条**可能在统一仓库里，也可能在系统或管理器里**——看 `source` 字段 |
| **要装一个本机没有的工具** | 先 `find` 确认本机确实没有（找不到会退出码 1）→ `install` 进统一仓库 → 自动登记进地图 |

地图是**索引**，仓库是**存放地点**。地图里没有这条工具，意味着这台机器上没有（或还没登记）；
